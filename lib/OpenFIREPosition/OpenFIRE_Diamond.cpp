...
void OpenFIRE_Diamond::begin(const int* px, const int* py, unsigned int seen) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
        // if LED not seen...
        if (!(seenFlags & (1 << i))) {
            // if unseen make sure all quadrants have a value if missing apply value with buffer and set to unseen (this
            // step is important for 1 LED usage)
            if (!(((positionY[0] < medianY - (height2 / 2) + buff) &&
                   (tilt ? positionX[0] >= medianX - (width2 / 2) + buff
                         : positionX[0] < medianX + (width2 / 2) - buff)) ||
                  ((positionY[1] < medianY - (height2 / 2) + buff) &&
                   (tilt ? positionX[1] >= medianX - (width2 / 2) + buff
                         : positionX[1] < medianX + (width2 / 2) - buff)) ||
                  ((positionY[2] < medianY - (height2 / 2) + buff) &&
                   (tilt ? positionX[2] >= medianX - (width2 / 2) + buff
                         : positionX[2] < medianX + (width2 / 2) - buff)) ||
                  ((positionY[3] < medianY - (height2 / 2) + buff) &&
                   (tilt ? positionX[3] >= medianX - (width2 / 2) + buff
            ...
            if (!((positionX[0] > medianX + (width2 / 2) - buff) || (positionX[1] > medianX + (width2 / 2) - buff) ||
                  (positionX[2] > medianX + (width2 / 2) - buff) || (positionX[3] > medianX + (width2 / 2) - buff))) {
                float f = angle;
            ...
            if (positionY[i] < (medianY - (height2 / 2) + buff) &&
                (tilt ? positionX[i] >= medianX - (width2 / 2) + buff : positionX[i] < medianX + (width2 / 2) - buff)) {
                float f = angle + fHPI;
                positionX[i] = medianX - round((height / 2) * cos(f));
                positionY[i] = medianY - round((height / 2) * sin(f)) - buff;
                see[0] = 0;
            ...
            if (positionYY[i] < (medianY - (height2 / 2) + buff) &&
                (tilt ? positionXX[i] >= medianX - buff : positionXX[i] < medianX + buff)) {
        ...
        if (positionY[i] < (medianY - (height2 / 2)) &&
            (tilt ? positionX[i] >= medianX - buff : positionX[i] < medianX + buff)) {
            if (see[0] & 0x02) {
