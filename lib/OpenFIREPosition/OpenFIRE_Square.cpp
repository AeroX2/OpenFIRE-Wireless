...
void OpenFIRE_Square::begin(const int* px, const int* py, unsigned int seen) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
        ...
        if (!(seenFlags & (1 << i))) {
            ...
            if (!(((positionY[0] < medianY) && (positionX[0] < medianX)) ||
                  ((positionY[1] < medianY) && (positionX[1] < medianX)) ||
                  ((positionY[2] < medianY) && (positionX[2] < medianX)) ||
                  ((positionY[3] < medianY) && (positionX[3] < medianX)))) {
    ...
    // Add tilt correction
    angle =
        (atan2(FinalY[0] - FinalY[1], FinalX[1] - FinalX[0]) + atan2(FinalY[2] - FinalY[3], FinalX[3] - FinalX[2])) /
        2.0f;
