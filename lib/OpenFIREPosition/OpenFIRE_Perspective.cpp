...
void multMats(float* a, float* b, float* res) {
    for (int r = 0; r < 4; r++) {
        int ri = r * 4;
        for (int c = 0; c < 4; c++) {
            res[ri + c] = (a[ri + 0] * b[c + 0] + a[ri + 1] * b[c + 4] + a[ri + 2] * b[c + 8] + a[ri + 3] * b[c + 12]);
        }
    }
}

void OpenFIRE_Perspective::warp(int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, float dx0, float dy0,
    ...
    computeQuadToSquare(srcmatrix, float(x0), float(y0), float(x1), float(y1), float(x2), float(y2), float(x3),
                        float(y3));
