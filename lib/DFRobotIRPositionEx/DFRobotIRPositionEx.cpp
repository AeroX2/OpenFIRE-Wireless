...
bool DFRobotIRPositionEx::begin(uint32_t clock, DataFormat_e format, Sensitivity_e sensitivity) {
    ...
    // For ESP32_S3 the clock must be changed after begin()
    wire.begin();
    wire.setClock(clock);
#else
...
int DFRobotIRPositionEx::basicAtomic(DFRobotIRPositionEx::Retry_e retry) {
    ...
    for (unsigned int i = 0, retries = retry >> 1; i <= retries; ++i) {
        ...
        if (!memcmp(&positionData[0].receivedBuffer[1], &positionData[1].receivedBuffer[1],
                    DFRIRdata_LengthBasic - 1)) {
