...
class TinyUSBDevices_ {
   public:
    ...
    enum reportChannels_e { reportMouse = 0, reportKeyboard, reportGamepad };
...
uint8_t const desc_hid_report[] = {TUD_HID_REPORT_DESC_KEYBOARD(HID_REPORT_ID(HID_RID_e::HID_RID_KEYBOARD)),
     ...
     *****************************/
        ...
        #define MOUSE_LEFT hid_mouse_button_bm_t::MOUSE_BUTTON_LEFT    // 0x01    // 696969 defined in TinyUSB in hid.h
        #define MOUSE_RIGHT hid_mouse_button_bm_t::MOUSE_BUTTON_RIGHT  // 0x02    // 696969 defined in TinyUSB in hid.h
        #define MOUSE_MIDDLE \
            hid_mouse_button_bm_t::MOUSE_BUTTON_MIDDLE  // 0x04    // 696969 defined in TinyUSB in hid.h
        #define MOUSE_BUTTON4 \
            hid_mouse_button_bm_t::MOUSE_BUTTON_BACKWARD  // 0x08    // 696969 defined in TinyUSB in hid.h
        #define MOUSE_BUTTON5 \
            hid_mouse_button_bm_t::MOUSE_BUTTON_FORWARD  // 0x10    // 696969 defined in TinyUSB in hid.h
...
class AbsMouse5_ {
   ...
   public:
    AbsMouse5_();  // can be removed, not needed for anything
    void report(void);
    void move(int16_t x, int16_t y);
    void move_wheel_pan(int8_t wheel, int8_t pan);                       // NOT ESSENTIAL ADDED BY ME
    void press(uint8_t b = hid_mouse_button_bm_t::MOUSE_BUTTON_LEFT);    // only one button at a time
    void release(uint8_t b = hid_mouse_button_bm_t::MOUSE_BUTTON_LEFT);  // only one button at a time
    void releaseAll() {
        release(0x1f);
    }
    void click(uint8_t b = hid_mouse_button_bm_t::MOUSE_BUTTON_LEFT);  // clicks all buttons as set and then
                                                                       // releases them immediately - evaluate if to
                                                                       // keep it // NOT ESSENTIAL ADDED BY ME
    void buttons(uint8_t b);  // sets all buttons together without doing it one by one or disables them - evaluate if
                              // to keep it // NOT ESSENTIAL ADDED BY ME
    bool isPressed(uint8_t b = hid_mouse_button_bm_t::MOUSE_BUTTON_LEFT);  // NOT ESSENTIAL ADDED BY ME
};
...
extern AbsMouse5_ AbsMouse5;
    ...
    //  are compatible with Arduino Keyboard.h API
        ...
        #define KEY_ESC \
            0xB1  // 696969 doesn't correspond to TinyUSB's HID_KEY_ESCAPE, but let's leave it as set, huh ???
...
class Keyboard_ : public Print {
   ...
   public:
    Keyboard_(void);
    void report(void);
    size_t write(uint8_t k);                           // never used but needed for print
    size_t write(const uint8_t *buffer, size_t size);  // never used but needed for print
...
extern Keyboard_ Keyboard;
     ...
     *****************************/
        ...
        // THE BUTTON CODES FOR THE DPAD (HAT) START FROM 32 OR AFTER THE 32 (0-31) CODES FOR THE BUTTONS
        #define PAD_UP 32     // from here the codes for DPad/hat start
        #define PAD_DOWN 33   //       "                 "
        #define PAD_LEFT 34   //       "                 "
        #define PAD_RIGHT 35  //       "                 "

        #define START_DPAD_KEY PAD_UP  // used as reference position for lightgunButtons.cpp file
