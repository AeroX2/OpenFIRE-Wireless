 ...
 * which in itself is based on the 4IR Beta "Big Code Update" SAMCO project from
...
// We're setting our custom USB identifiers, as defined in the configuration area!
    ...
    // ===================================================================================================================
    // ===================================================================================================================
    // ====== 696969 ============ moved above before connection
    // =================================================
...
////////////////////////////////////////////////////////////
// writes "connection" on the top part of the display
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    // === 696969 === END NEW USB INITIALIZATION OR WIRELESS CONNECTION MANAGEMENT ========================
    //////////////////////////////////////////////////////////////////////////////////////////////////////////
    #endif
#endif  // USE_TINYUSB

// restore the top part of the display as in original
#ifdef ARDUINO_ARCH_ESP32
    #ifdef USES_DISPLAY
...
void loop1() {
    ...
    while (FW_Common::gunMode == FW_Const::GunMode_Run) {
        ...
        if (bitRead(FW_Common::buttons.debounced,
                    FW_Const::BtnIdx_Trigger))  // Check if we pressed the Trigger this run.
            ...
            if (FW_Common::buttons.debounced == FW_Const::EnterPauseModeHoldBtnMask && !FW_Common::lastSeen &&
                ...
                if (!OF_Serial::serialMode)
                    Serial.println("Started holding pause mode signal buttons!");
                       ...
                       (FW_Common::buttons.debounced != FW_Const::EnterPauseModeHoldBtnMask || FW_Common::lastSeen)) {
                pauseHoldStarted = false;
                if (!OF_Serial::serialMode)
                    Serial.println(
                        "Either stopped holding pause mode buttons, aimed onscreen, or pressed other buttons");
    ...
    switch (FW_Common::gunMode) {
        case FW_Const::GunMode_Pause:
            if (OF_Prefs::toggles[OF_Const::simplePause]) {
...
#ifdef USES_DISPLAY
                            FW_Common::OLED.PauseProfileUpdate(profileModeSelection, OF_Prefs::profiles[0].name,
                                                               OF_Prefs::profiles[1].name, OF_Prefs::profiles[2].name,
                ...
                FW_Common::SetRunMode(FW_Common::runMode == FW_Const::RunMode_Average ? FW_Const::RunMode_Average2
                                                                                      : FW_Const::RunMode_Average);
                ...
                FW_Common::SavePreferences();
                       ...
                       OF_Prefs::pins[OF_Const::rumbleSwitch] >= 0) {
                RumbleToggle();
#endif  // USES_RUMBLE
#ifdef USES_SOLENOID
            } else if (FW_Common::buttons.pressedReleased == FW_Const::SolenoidToggleBtnMask &&
                       OF_Prefs::pins[OF_Const::solenoidSwitch] >= 0) {
                SolenoidToggle();
#endif  // USES_SOLENOID
            } else
                SelectCalProfileFromBtnMask(FW_Common::buttons.pressedReleased);
...
// Only sets these values if the switches are mapped to valid pins.
    ...
    #ifdef MAMEHOOKER
            // Solenoid feedback on the second core is hella wonky when ammo updates are performed there likely due to
            // blocking I2C transactions, so just do it here using the signal sent by it.
        ...
        if (!OF_FFB::triggerHeld) {
            if (FW_Common::buttons.analogOutput)  // this should only ever be gamepad outputs
                ...
                switch (LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportType) {
                    case LightgunButtons::ReportType_Mouse:
                        AbsMouse5.press(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode);
                        break;
                    case LightgunButtons::ReportType_Keyboard:
                        Keyboard.press(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode);
                        break;
                    case LightgunButtons::ReportType_Gamepad:
                        Gamepad16.press(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode);
                        break;
                    default:
                        break;
                }
        ...
        if (!OF_FFB::triggerHeld) {
            if (FW_Common::buttons.analogOutput)  // always uses the same button
                ...
                switch (LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportType2) {
                    case LightgunButtons::ReportType_Mouse:
                        AbsMouse5.press(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode2);
                        break;
                    case LightgunButtons::ReportType_Keyboard:
                        Keyboard.press(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode2);
                        break;
                    case LightgunButtons::ReportType_Gamepad:
                        Gamepad16.press(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode2);
                        break;
                    default:
                        break;
                }
...
void TriggerNotFire() {
    if (OF_FFB::triggerHeld) {
        if (FW_Common::buttons.analogOutput)
            ...
            switch (LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportType2) {
                case LightgunButtons::ReportType_Mouse:
                    AbsMouse5.release(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode2);
                    break;
                case LightgunButtons::ReportType_Keyboard:
                    Keyboard.release(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode2);
                    break;
                case LightgunButtons::ReportType_Gamepad:
                    Gamepad16.release(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode2);
                    break;
                default:
                    break;
            }
            ...
            switch (LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportType) {
                case LightgunButtons::ReportType_Mouse:
                    AbsMouse5.release(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode);
                    break;
                case LightgunButtons::ReportType_Keyboard:
                    Keyboard.release(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode);
                    break;
                case LightgunButtons::ReportType_Gamepad:
                    Gamepad16.release(LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportCode);
                    break;
                default:
                    break;
            }
...
void SendEscapeKey() {
    Keyboard.press(KEY_ESC);  // 696969 doesn't correspond to HID_KEY_ESCAPE, but let's leave it as set, huh ?
    Keyboard.report();
    delay(20);                  // wait a bit so it registers on the PC.
    Keyboard.release(KEY_ESC);  // 696969 doesn't correspond to HID_KEY_ESCAPE, but let's leave it as set, huh ?
    Keyboard.report();
    lastUSBpoll = millis();
}
...
#ifdef USES_DISPLAY
    FW_Common::OLED.PauseProfileUpdate(profileModeSelection, OF_Prefs::profiles[0].name, OF_Prefs::profiles[1].name,
                                       OF_Prefs::profiles[2].name, OF_Prefs::profiles[3].name);
#endif  // USES_DISPLAY
...
void BurstFireToggle()
{
    ...
    if(burstFireActive) {  // Did we flick it on?
        ...
        #ifdef USES_SOLENOID
            for(byte i = 0; i < 4; ++i) {
                digitalWrite(solenoidPin, HIGH);                  // Demonstrate it by flicking the solenoid on/off
three times! delay(OF_Prefs::settings[OF_Const::solenoidFastInterval]);                      // (at a fixed rate to
distinguish it from autofire speed toggles) digitalWrite(solenoidPin, LOW);
                delay(OF_Prefs::settings[OF_Const::solenoidFastInterval] * 2);
            }
        #endif // USES_SOLENOID
        #ifdef LED_ENABLE
            OF_RGB::SetLedPackedColor(OF_Prefs::profiles[OF_Prefs::currentProfile].color);// And reset the LED back to
pause mode color #endif // LED_ENABLE return; } else {  // Or we flicked it off. Serial.println("Burst firing
disabled!"); #ifdef LED_ENABLE OF_RGB::SetLedPackedColor(WikiColor::Orange); #endif // LED_ENABLE #ifdef USES_SOLENOID
        ...
        #ifdef LED_ENABLE
            OF_RGB::SetLedPackedColor(OF_Prefs::profiles[OF_Prefs::currentProfile].color);// And reset the LED back to
pause mode color #endif // LED_ENABLE return;
    }
}
*/
...
void RumbleToggle() {
    ...
    if (OF_Prefs::toggles[OF_Const::rumble]) {
        ...
        OF_RGB::SetLedPackedColor(
            OF_Prefs::profiles[OF_Prefs::currentProfile].color);  // And reset the LED back to pause mode color
    #endif                                                        // LED_ENABLE
