...
void OF_Serial::SerialProcessing() {
        ...
        case 'S':
            ...
            // TODO: handle the other Start line bits? this assume equiv of `S6`
                ...
                if (FW_Common::gunMode == FW_Const::GunMode_Run)
                    FW_Common::OLED.ScreenModeChange(ExtDisplay::Screen_Mamehook_Single,
                                                     FW_Common::buttons.analogOutput);
    #endif  // USES_DISPLAY
            }
            break;
        // Modesetting Signal
                ...
                case '2':
                    ...
                    switch (Serial.read()) {
                        ...
                        case '1':
                            ...
                            break;
                        // make middle mouse button (mapping of Button B, useful for low buttons mode & e.g. using VCop3
                        // ES mode)
                        case '2':
                            memcpy(&LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Pedal].reportType,
                ...
                case '3':
                    ...
                    if (!serialMode) {
                        if (serialARcorrection) {
                            Serial.println("Setting 4:3 correction on!");
                        } else {
                            Serial.println("Setting 4:3 correction off!");
                        }
                    }
                    break;
    #ifdef USES_TEMP
            ...
            // autoreload (TODO: maybe?)
                ...
                case '6':
                    ...
                    switch (Serial.read()) {
                        // disable
                        case '0':
                            if (OF_Prefs::pins[OF_Const::solenoidSwitch] == -1 &&
                        ...
                        // enable
                        case '1':
                            if (OF_Prefs::pins[OF_Const::rumbleSwitch] == -1 &&
                                OF_Prefs::pins[OF_Const::rumblePin] >= 0) {
                                OF_Prefs::toggles[OF_Const::rumble] = true;
                            }
                            if (OF_Prefs::pins[OF_Const::solenoidSwitch] == -1 &&
                                OF_Prefs::pins[OF_Const::solenoidPin] >= 0) {
                                OF_Prefs::toggles[OF_Const::solenoid] = false;
                            }
                            if (OF_Prefs::pins[OF_Const::rumblePin] >= 0) {
                                OF_Prefs::toggles[OF_Const::rumbleFF] = true;
                            }
                            break;
                    }
                    OF_FFB::FFBShutdown();
            ...
            switch (Serial.read()) {
                ...
                case 'R': {
                    ...
                    if (serialInput >= '1' && serialInput <= '4') {
                        ...
                        Serial.println(
                            "SERIALREAD: Player remap command called, but an invalid or no slot number was declared!");
                    break;
                }
                default:
    ...
    #if /*defined(ARDUINO_ARCH_RP2040) &&*/ defined(DUAL_CORE)  // This may be being run from Core 1, so signal if
                                                                // running in main Run Mode.
                if (FW_Common::gunMode == FW_Const::GunMode_Run) {
        #ifdef ARDUINO_ARCH_ESP32
            ...
            switch (Serial.read()) {
                ...
                case '2':
                    ...
                    switch (Serial.read()) {
                            ...
                            if (!serialQueue[SerialQueue_LEDPulse]) {
                                ...
                                if (Serial.peek() >= '0' && Serial.peek() <= '9') {
                                    ...
                                    serialLEDPulseColorMap =
                                        0b00000001;  // Set the R LED as the one pulsing only (overwrites the others).
                                    char serialInputS[4] = {0, 0, 0, 0};
                                    for (uint n = 0; n < 3; ++n) {
                                        serialInputS[n] = Serial.read();
                                        if (Serial.peek() < '0' || Serial.peek() > '9')
                        ...
                        // LED Red "off" command
                        case '0':
                            serialLEDChange = true, serialQueue[SerialQueue_Red] = false,
                            serialQueue[SerialQueue_LEDPulse] = false, serialLEDR = 0, serialLEDPulseColorMap = 0;
                            break;
                    }
                    break;
                // LED Green bits
                case '3':
                    ...
                    switch (Serial.read()) {
                        ...
                        case '2':
                            if (!serialQueue[SerialQueue_LEDPulse]) {
                                ...
                                if (Serial.peek() >= '0' && Serial.peek() <= '9') {
                                    serialLEDChange = true, serialQueue[SerialQueue_LEDPulse] = true,
                                    serialLEDPulseColorMap =
                                        0b00000010;  // Set the G LED as the one pulsing only (overwrites the others).
                                    char serialInputS[4] = {0, 0, 0, 0};
                                    for (uint n = 0; n < 3; ++n) {
                                        serialInputS[n] = Serial.read();
                                        if (Serial.peek() < '0' || Serial.peek() > '9')
                        ...
                        case '0':
                            serialLEDChange = true, serialQueue[SerialQueue_Green] = false,
                            serialQueue[SerialQueue_LEDPulse] = false, serialLEDG = 0, serialLEDPulseColorMap = 0;
                            break;
                    }
                    break;
                // LED Blue bits
                case '4':
                    ...
                    switch (Serial.read()) {
                        ...
                        case '2':
                            if (!serialQueue[SerialQueue_LEDPulse]) {
                                ...
                                if (Serial.peek() >= '0' && Serial.peek() <= '9') {
                                    serialLEDChange = true, serialQueue[SerialQueue_LEDPulse] = true,
                                    serialLEDPulseColorMap =
                                        0b00000100;  // Set the B LED as the one pulsing only (overwrites the others).
                                    char serialInputS[4] = {0, 0, 0, 0};
                                    for (uint n = 0; n < 3; ++n) {
                                        serialInputS[n] = Serial.read();
                                        if (Serial.peek() < '0' || Serial.peek() > '9')
                        ...
                        case '0':
                            serialLEDChange = true, serialQueue[SerialQueue_Blue] = false,
                            serialQueue[SerialQueue_LEDPulse] = false, serialLEDB = 0, serialLEDPulseColorMap = 0;
                            break;
                    }
                    break;
    #endif  // LED_ENABLE
                        ...
                        case 'L':
                            ...
                            if (Serial.peek() >= '0' && Serial.peek() <= '9') {
                                ...
                                if (FW_Common::OLED.lifeBar) {
                                    ...
                                    FW_Common::dispLifePercentage =
                                        (100 * serialLifeCount) /
                                        FW_Common::dispMaxLife;  // Calculate the Life % to show
                                }
                                serialDisplayChange = true;
                            }
                            break;
                ...
                default:
                    // Serial.println("SERIALREAD: Feedback command detected, but no feedback devices are built into
                    // this firmware!");
                    break;
    #endif
            }
...
void OF_Serial::SerialHandling() {
    ...
    // Solenoid "normal" enable bits need to be monitored to ensure it isn't on for too long, and force shutdown if it
    // is. Solenoid "pulse" bits will borrow from current force feedback settings. Rumble pulse bits are also something
        ...
        if (serialQueue[SerialQueue_Solenoid]) {
            ...
            if (!serialSolPulsesLast) {                                     // Have we started pulsing?
                ...
                if (digitalRead(OF_Prefs::pins[OF_Const::solenoidPin])) {
                    ...
                    } else if (millis() - serialSolPulsesLastUpdate >= OF_Prefs::settings[OF_Const::solenoidOffLength]
                                                                           << OF_FFB::autofireDoubleLengthWait
                                   ? 1
                                   : 0) {
                        digitalWrite(OF_Prefs::pins[OF_Const::solenoidPin], HIGH);  // Start pulsing it on.
                        serialSolPulsesLastUpdate = millis();                       // Timestamp our last pulse event.
    ...
    if (OF_Prefs::toggles[OF_Const::rumble]) {
        ...
        if (serialQueue[SerialQueue_Rumble]) {
            analogWrite(OF_Prefs::pins[OF_Const::rumblePin],
                        OF_Prefs::settings[OF_Const::rumbleStrength]);  // turn/keep it on.
            ...
            if (!serialRumbPulsesLast) {
                ...
                if (serialRumbCustomHoldLength && serialRumbCustomPauseLength) {
                ...
                } else if (millis() - serialRumbPulsesLastUpdate >
                           serialRumbPulsesLength) {  // have we waited enough time between pulse stages?
                    switch (serialRumbPulseStage) {
                        ...
                        case 1:
                            analogWrite(OF_Prefs::pins[OF_Const::rumblePin],
                                        OF_Prefs::settings[OF_Const::rumbleStrength] / 2);
                            serialRumbPulseStage++;
                            serialRumbPulsesLastUpdate = millis();
                            break;
                        // Falloff
                        case 2:
                            analogWrite(OF_Prefs::pins[OF_Const::rumblePin],
                                        OF_Prefs::settings[OF_Const::rumbleStrength] / 3);
                            serialRumbPulseStage++;
                            serialRumbPulsesLastUpdate = millis();
                            break;
                        // Check
                        case 3:
                            if (serialRumbPulsesLast >= serialRumbPulses) {
                                ...
                                serialRumbPulsesLast++, serialRumbPulseStage = 0;
                            break;
                    }
                }
            }
    ...
    if (serialLEDChange) {  // Has the LED command state changed?
        ...
        if (serialQueue[SerialQueue_LEDPulse]) {
            ...
            if (!serialLEDPulsesLast) {  // Are we just starting?
                serialQueue[SerialQueue_Red] = false, serialQueue[SerialQueue_Green] = false,
                serialQueue[SerialQueue_Blue] = false;
                ...
                if (millis() - serialLEDPulsesLastUpdate >
                    serialLEDPulsesLength) {           // have we waited enough time between pulse stages?
                    switch (serialLEDPulseColorMap) {  // Check the color map
                        case 0b00000001:               // Basically for R, G, or B,
                            if (serialLEDPulseRising) {
                                serialLEDR += 3;  // Set the LED value up by three (it's easiest to do blindly like this
                                                  // without over/underflowing tbh)
                                ...
                                if (serialLEDR == 0) {  // If the LED value has reached the lowest point,
                                    serialLEDPulseRising =
                                        true;               // Set that we should be in the rising part of a new cycle.
                ...
                serialLEDPulseColorMap = 0b00000000, serialQueue[SerialQueue_LEDPulse] = false;
            // Any LED static bits
...
void OF_Serial::SerialProcessingDocked() {
    ...
    switch (c) {
        ...
        case OF_Const::sGetToggles:
            SerialBatchSend(OF_Prefs::toggles, OF_Prefs::OFPresets.boolTypes_Strings,
                            sizeof(OF_Prefs::toggles) / OF_Const::boolTypesCount);
            break;
        case OF_Const::sGetPins:
            SerialBatchSend(OF_Prefs::pins, OF_Prefs::OFPresets.boardInputs_Strings,
                            sizeof(OF_Prefs::pins) / OF_Const::boardInputsCount);
            break;
        case OF_Const::sGetSettings:
            SerialBatchSend(OF_Prefs::settings, OF_Prefs::OFPresets.settingsTypes_Strings,
                            sizeof(OF_Prefs::settings) / OF_Const::settingsTypesCount);
            break;
        case OF_Const::sGetBtns:
            SerialBatchSend(OF_Prefs::backupButtonDesc, OF_Prefs::OFPresets.boardInputs_Strings,
                            sizeof(OF_Prefs::backupButtonDesc) / ButtonCount);
            break;
        case OF_Const::sGetProfile:
            for (int prof = 0; prof < PROFILE_COUNT; ++prof)
                SerialBatchSend(&OF_Prefs::profiles[prof], OF_Prefs::OFPresets.profSettingTypes_Strings,
                                sizeof(uint32_t), prof);
            break;
        ...
        // though this might just load corrupt data instead. :shrug:
                            ...
                            default:
                                ...
                                datSize = Serial.read();
                                RXbuf[rxLen++] = datSize;
                                if (type == OF_Const::sCommitProfile &&
                                    (OF_Prefs::OFPresets.profSettingTypes_Strings.count(RXbuf) == 0 ||
                                     (OF_Prefs::OFPresets.profSettingTypes_Strings.count(RXbuf) &&
                                      OF_Prefs::OFPresets.profSettingTypes_Strings.at(RXbuf) !=
                                          OF_Const::profCurrent))) {
                                    profNum = Serial.read();
                                    RXbuf[rxLen++] = profNum;
                                }
                                ...
                                switch (type) {
                                    case OF_Const::sCommitToggles:
                                        SerialBatchRecv(RXbuf, OF_Prefs::toggles, OF_Prefs::OFPresets.boolTypes_Strings,
                                                        sizeof(OF_Prefs::toggles) / OF_Const::boolTypesCount, datSize,
                                                        rxLen);
                                        break;
                                    case OF_Const::sCommitPins:
                                        SerialBatchRecv(RXbuf, OF_Prefs::pins, OF_Prefs::OFPresets.boardInputs_Strings,
                                                        sizeof(OF_Prefs::pins) / OF_Const::boardInputsCount, datSize,
                                                        rxLen);
                                        break;
                                    case OF_Const::sCommitSettings:
                                        SerialBatchRecv(
                                            RXbuf, OF_Prefs::settings, OF_Prefs::OFPresets.settingsTypes_Strings,
                                            sizeof(OF_Prefs::settings) / OF_Const::settingsTypesCount, datSize, rxLen);
                                        break;
                                    case OF_Const::sCommitBtns:
                                        SerialBatchRecv(
                                            RXbuf, OF_Prefs::backupButtonDesc, OF_Prefs::OFPresets.boardInputs_Strings,
                                            sizeof(OF_Prefs::backupButtonDesc) / ButtonCount, datSize, rxLen);
                                        break;
                                    case OF_Const::sCommitProfile:
                                        if (profNum < PROFILE_COUNT)
                                            SerialBatchRecv(RXbuf, &OF_Prefs::profiles[profNum],
                                                            OF_Prefs::OFPresets.profSettingTypes_Strings,
                                                            sizeof(uint32_t), datSize, rxLen);
                                        break;
                                    default:
                                        break;
                                }
        ...
        case OF_Const::sClearFlash:
            if (Serial.available() == 1 && Serial.read() == OF_Const::sClearFlash) {
                ...
                if (TinyUSBDevices.onBattery) {
                    // send reboot code also to the dongle device.. decide ???
                }
    #endif
                ESP.restart();
#else
...
void OF_Serial::SerialBatchSend(void *dataPtr, const std::unordered_map<std::string_view, int> &mapPtr,
                                const size_t &dataSize, const int &profNum) {
    size_t pos;
            ...
            if (&mapPtr == &OF_Prefs::OFPresets.profSettingTypes_Strings) {
                if (pair.second == OF_Const::profCurrent) {
                    ...
                    if (pair.second == OF_Const::profName) {
                        ...
                        TXbuf[pos++] = profNum;
                        memcpy(&TXbuf[pos], (uint8_t *)dataPtr + (dataSize * pair.second),
                ...
                if (dataPtr == OF_Prefs::backupButtonDesc && pair.second >= ButtonCount - 1)
                    continue;
                TXbuf[pos++] = dataSize;
                memcpy(&TXbuf[pos], (uint8_t *)dataPtr + (dataSize * pair.second), dataSize);
                pos += dataSize;
...
void OF_Serial::SerialBatchRecv(const char *bufPtr, void *dataPtr,
                                const std::unordered_map<std::string_view, int> &mapPtr, const size_t &dataSize,
                                const size_t &rxDatSize, const size_t &rxBufSize) {
...
// #ifdef ARDUINO_ARCH_RP2040 // 696969 for ESP32
#ifdef DUAL_CORE
            Serial.println("Running on dual cores.");
#else
            Serial.println("Running on one core.");
#endif  // DUAL_CORE
        // #endif // ARDUINO_ARCH_RP2040 // 696969 for ESP32

            Serial.printf(
                "Firmware version: v%.1f"
#ifdef GIT_HASH
                "-%s"
#endif  // GIT_ HASH);
                ,
