...
/* TODO: Arduino Nano LED support disabled due to instability.
    ...
    #define LED_ENABLE
    // Apparently an LED is included, but has to be communicated with through the WiFi chip (or else it throws compiler
errors)
...
class OF_RGB {
   public:
    ...
    static uint8_t Invert(const uint8_t &orig) {
        return ~orig;
    }
