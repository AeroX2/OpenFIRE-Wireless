...
void OF_RGB::SetLedPackedColor(const uint32_t &color) {
    ...
    if (externPixel != nullptr) {
        if (OF_Prefs::settings[OF_Const::customLEDstatic] < OF_Prefs::settings[OF_Const::customLEDcount]) {
            if (OF_Prefs::toggles[OF_Const::invertStaticPixels])
                externPixel->fill(
                    color, 0,
                    OF_Prefs::settings[OF_Const::customLEDcount] - OF_Prefs::settings[OF_Const::customLEDstatic]);
            else
    ...
    /* Arduino Nano LED support disabled due to instability.
    #ifdef ARDUINO_NANO_RP2040_CONNECT
        // in case the color bytes were already flipped before, as Arduino Nano also uses power sink pins i.e. common
    anode if(FW_Common::ledIsValid && !OF_Prefs::toggles.commonAnode) { r = ~r; g = ~g; b = ~b;
        }
        analogWrite(LEDR, r);
        analogWrite(LEDG, g);
        analogWrite(LEDB, b);
