...
class OF_FFB {
   public:
    ...
    /// @details Only polls every 250ms, with updates committed to temperatureCurrent after four successful polling
    /// cycles
    ...
    enum TempStatuses_e { Temp_Safe = 0, Temp_Warning, Temp_Fatal };

    static inline TempStatuses_e tempStatus =
        Temp_Safe;  // Relative current state of the solenoid, according to TempStatuses_e
...
#define TEMP_UPDATE_INTERVAL 250
   ...
   private:
    ...
    // For rumble:
    static inline bool rumbleHappened =
        false;  // If we're holding, this marks we sent a rumble command already; is cleared when trigger is released

    static inline unsigned long previousMillisSol = 0;  // Timestamp of last time since unique solenoid state change

    // timer stuff
    static inline unsigned long currentMillis =
        0;  // Current millis() value, which is globally updated/read across all functions in this class
    static inline unsigned long previousMillisTemp = 0;  // Timestamp of last time TMP36 was read

    static inline int temperatureGraph[4];  // Table of collected (converted) TMP36 readings, to be averaged into
                                            // temperatureCurrent on the fourth value.
    static inline uint tempGraphIndex =
        0;  // Current index of temperatureGraph to update; initiates temperatureCurrent update/averaging when = 3.
