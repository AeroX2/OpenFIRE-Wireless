...
class OF_Serial {
   public:
    ...
    static void SerialBatchSend(void *, const std::unordered_map<std::string_view, int> &, const size_t &,
                                const int & = -1);

    /// @brief    Generic method for reading commit data over Serial from connected host
    static void SerialBatchRecv(const char *, void *, const std::unordered_map<std::string_view, int> &, const size_t &,
    ...
    static inline bool serialQueue[SerialQueueBitsCount] = {
        false};                                     // Array of events we've queued from the serial receipt.
    static inline bool serialARcorrection = false;  // 4:3 AR correction mode flag
    static inline bool serialMappingsOffscreenShot =
        false;  // Marker if Offscreen Shot Mode's been enabled, for FW_Common::UpdateBindings
    ...
    #ifdef USES_DISPLAY
    static inline bool serialDisplayChange =
        false;  // Signal of pending display update, sent by Core 2 to be used by Core 1 in dual core configs
   ...
   private:
    ...
    #ifdef LED_ENABLE
    static inline unsigned long serialLEDPulsesLastUpdate =
        0;  // The timestamp of the last serial-invoked LED pulse update we iterated.
    static inline unsigned int serialLEDPulsesLength =
        2;                                           // How long each stage of a serial-invoked pulse rumble is, in ms.
    static inline bool serialLEDChange = false;      // Set on if we set an LED command this cycle.
    static inline bool serialLEDPulseRising = true;  // In LED pulse events, is it rising now? True to indicate rising,
                                                     // false to indicate falling; default to on for very first pulse.
    ...
    static inline uint8_t serialLEDPulseColorMap = 0b00000000;  // The map of what LEDs should be pulsing (we use the
                                                                // rightmost three of this bitmask for R, G, or B).
    #endif                                                      // LED_ENABLE

    #ifdef USES_RUMBLE
    static inline unsigned long serialRumbPulsesLastUpdate =
        0;  // The timestamp of the last serial-invoked pulse rumble we updated.
    static constexpr uint serialRumbPulsesLength =
        60;  // How long each stage of a serial-invoked pulse rumble is, in ms.
    static inline uint serialRumbPulseStage =
        0;                                    // 0 = start/rising, 1 = peak, 2 = falling, 3 = final check/reset to start
    ...
    static inline uint serialRumbCustomHoldLength =
        0;  // Determines custom solenoid ON state length for sol "pulse" commands - 0 = use system settings
    static inline uint serialRumbCustomPauseLength =
        0;  // Determines custom solenoid OFF state length for sol "pulse" commands - 0 = use system settings
    #endif  // USES_RUMBLE

    #ifdef USES_SOLENOID
    static inline unsigned long serialSolPulsesLastUpdate =
        0;  // The timestamp of the last serial-invoked pulse solenoid event we updated.
    ...
    static inline unsigned long serialSolTimestamp =
        0;  // Timestamp of the last solenoid static on command (for safety)
    static inline uint serialSolCustomHoldLength =
        0;  // Determines custom solenoid ON state length for sol "pulse" commands - 0 = use system settings
    static inline uint serialSolCustomPauseLength =
        0;  // Determines custom solenoid OFF state length for sol "pulse" commands - 0 = use system settings
        #ifdef USES_TEMP
