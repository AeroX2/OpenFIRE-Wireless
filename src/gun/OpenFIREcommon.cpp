...
void FW_Common::FeedbackSet() {
    ...
    if (OF_Prefs::pins[OF_Const::rumblePin] >= 0)
        ...
        OF_Prefs::toggles[OF_Const::rumble] = false;
#endif  // USES_RUMBLE
...
void FW_Common::CameraSet() {
#ifdef ARDUINO_ARCH_ESP32
    Wire.setPins(OF_Prefs::pins[OF_Const::camSDA], OF_Prefs::pins[OF_Const::camSCL]);  // MODIFIED 696969 for ESP32
    ...
    if (dfrIRPos != nullptr) {
        if (!dfrIRPos->begin(DFROBOT_IR_IIC_CLOCK, DFRobotIRPositionEx::DataFormat_Basic,
                             (DFRobotIRPositionEx::Sensitivity_e)OF_Prefs::profiles[OF_Prefs::currentProfile].irSens)) {
            delete dfrIRPos;
            dfrIRPos = nullptr;
            PrintIrError();
        } else
    ...
    switch (newMode) {
        ...
        case FW_Const::GunMode_Pause:
            ...
            if (OF_Prefs::toggles[OF_Const::simplePause])
                ...
                OLED.PauseScreenShow(OF_Prefs::currentProfile, OF_Prefs::profiles[0].name, OF_Prefs::profiles[1].name,
...
void FW_Common::ExecCalMode(const bool &fromDesktop) {
    ...
    while (gunMode == FW_Const::GunMode_Calibration) {
        ...
        if ((buttons.pressedReleased & (FW_Const::ExitPauseModeBtnMask | FW_Const::ExitPauseModeHoldBtnMask) ||
             Serial.read() == OF_Const::serialTerminator) &&
            !justBooted) {
                ...
                case FW_Const::Cali_Top:
                    ...
                    if (OF_Prefs::profiles[OF_Prefs::currentProfile].irLayout) {
                        OF_Prefs::profiles[OF_Prefs::currentProfile].adjX =
                            (OpenFIREdiamond.testMedianX() - (512 << 2)) * cos(OpenFIREdiamond.Ang()) -
                            (OpenFIREdiamond.testMedianY() - (384 << 2)) * sin(OpenFIREdiamond.Ang()) + (512 << 2);
                        OF_Prefs::profiles[OF_Prefs::currentProfile].adjY =
                            (OpenFIREdiamond.testMedianX() - (512 << 2)) * sin(OpenFIREdiamond.Ang()) +
                            (OpenFIREdiamond.testMedianY() - (384 << 2)) * cos(OpenFIREdiamond.Ang()) + (384 << 2);
                    } else {
                        OF_Prefs::profiles[OF_Prefs::currentProfile].adjX =
                            (OpenFIREsquare.testMedianX() - (512 << 2)) * cos(OpenFIREsquare.Ang()) -
                            (OpenFIREsquare.testMedianY() - (384 << 2)) * sin(OpenFIREsquare.Ang()) + (512 << 2);
                        OF_Prefs::profiles[OF_Prefs::currentProfile].adjY =
                            (OpenFIREsquare.testMedianX() - (512 << 2)) * sin(OpenFIREsquare.Ang()) +
                            (OpenFIREsquare.testMedianY() - (384 << 2)) * cos(OpenFIREsquare.Ang()) + (384 << 2);
                        // Work out LED locations by assuming height is 100%
                        OF_Prefs::profiles[OF_Prefs::currentProfile].TLled =
                            (res_x / 2) - ((OpenFIREsquare.W() * (res_y / OpenFIREsquare.H())) / 2);
                        OF_Prefs::profiles[OF_Prefs::currentProfile].TRled =
                            (res_x / 2) + ((OpenFIREsquare.W() * (res_y / OpenFIREsquare.H())) / 2);
                    }

                    // Update Cam centre in perspective library
                    OpenFIREper.source(OF_Prefs::profiles[OF_Prefs::currentProfile].adjX,
                                       OF_Prefs::profiles[OF_Prefs::currentProfile].adjY);
    ...
    if (OF_Prefs::toggles[OF_Const::rumble]) {
        ...
        delay(80);
    #ifdef ARDUINO_ARCH_ESP32
        analogWrite(OF_Prefs::pins[OF_Const::rumblePin], 0);  // 696969 for ESP32
    #else                                                     // rp2040
    ...
    if (dfrIRPos != nullptr) {
        ...
        if (error == DFRobotIRPositionEx::Error_Success) {
            ...
            if (OF_Prefs::profiles[OF_Prefs::currentProfile].irLayout) {  // layoutDiamond = 1
                ...
                OpenFIREper.warp(OpenFIREdiamond.X(0), OpenFIREdiamond.Y(0), OpenFIREdiamond.X(1), OpenFIREdiamond.Y(1),
                                 OpenFIREdiamond.X(2), OpenFIREdiamond.Y(2), OpenFIREdiamond.X(3), OpenFIREdiamond.Y(3),
                                 res_x / 2, 0, 0, res_y / 2, res_x / 2, res_y, res_x, res_y / 2);
            } else {  // layoutSquare = 0
                ...
                OpenFIREper.warp(OpenFIREsquare.X(0), OpenFIREsquare.Y(0), OpenFIREsquare.X(1), OpenFIREsquare.Y(1),
                                 OpenFIREsquare.X(2), OpenFIREsquare.Y(2), OpenFIREsquare.X(3), OpenFIREsquare.Y(3),
            ...
            mouseX = map(X_One_Euro, 0, res_x, (0 - OF_Prefs::profiles[OF_Prefs::currentProfile].leftOffset),
                         (res_x + OF_Prefs::profiles[OF_Prefs::currentProfile].rightOffset));
            mouseY = map(Y_One_Euro, 0, res_y, (0 - OF_Prefs::profiles[OF_Prefs::currentProfile].topOffset),
                         (res_y + OF_Prefs::profiles[OF_Prefs::currentProfile].bottomOffset));
    #endif
#endif
            ...
            */
            mouseX = map(X_Kalman, 0, res_x, (0 - OF_Prefs::profiles[OF_Prefs::currentProfile].leftOffset),
                         (res_x + OF_Prefs::profiles[OF_Prefs::currentProfile].rightOffset));
            mouseY = map(Y_Kalman, 0, res_y, (0 - OF_Prefs::profiles[OF_Prefs::currentProfile].topOffset),
                         (res_y + OF_Prefs::profiles[OF_Prefs::currentProfile].bottomOffset));
#else
            // Output mapped to screen resolution because offsets are measured in pixels
            mouseX = map(OpenFIREper.getX(), 0, res_x, (0 - OF_Prefs::profiles[OF_Prefs::currentProfile].leftOffset),
                         (res_x + OF_Prefs::profiles[OF_Prefs::currentProfile].rightOffset));
            mouseY = map(OpenFIREper.getY(), 0, res_y, (0 - OF_Prefs::profiles[OF_Prefs::currentProfile].topOffset),
                         (res_y + OF_Prefs::profiles[OF_Prefs::currentProfile].bottomOffset));
            ...
            if (gunMode == FW_Const::GunMode_Run) {
                ...
                if (OF_Serial::serialARcorrection)
                    switch (OF_Prefs::profiles[OF_Prefs::currentProfile].aspectRatio) {
...
int FW_Common::SavePreferences() {
    ...
    if (OF_Prefs::SaveProfiles() == OF_Prefs::Error_Success) {
        ...
        if (gunMode != FW_Const::GunMode_Docked)
            ...
            Serial.printf("%c%c (Successfully saved to LittleFS Storage)", OF_Const::sSave, true), Serial.flush();
        ...
        if (gunMode != FW_Const::GunMode_Docked)
            ...
            Serial.printf("%c%c (Failed to save to LittleFS Storage)", OF_Const::sSave, false), Serial.flush();
...
void FW_Common::UpdateBindings(const bool &rebindStrSel) {
    ...
    for (int i = 0; i < ButtonCount; ++i)
        memcpy(&LightgunButtons::ButtonDesc[i].reportType, OF_Prefs::backupButtonDesc[i],
    ...
    if (OF_Prefs::toggles[OF_Const::lowButtonsMode]) {
        memcpy(&LightgunButtons::ButtonDesc[FW_Const::BtnIdx_A].reportType2,
               OF_Prefs::backupButtonDesc[FW_Const::BtnIdx_Start], 2);
        memcpy(&LightgunButtons::ButtonDesc[FW_Const::BtnIdx_B].reportType2,
               OF_Prefs::backupButtonDesc[FW_Const::BtnIdx_Select], 2);
    }

#ifdef MAMEHOOKER
    if (OF_Serial::serialMappingsOffscreenShot) {
        memcpy(&LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Trigger].reportType2,
               OF_Prefs::backupButtonDesc[FW_Const::BtnIdx_A], sizeof(LightgunButtons::Desc_s::reportType) * 2);
    ...
    if (OF_Serial::serialMappingsPedalMode)
        switch (OF_Serial::serialMappingsPedalMode) {
            case 1:
                memcpy(&LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Pedal].reportType,
                       OF_Prefs::backupButtonDesc[FW_Const::BtnIdx_A], sizeof(OF_Prefs::backupButtonDesc[0]));
                break;
            case 2:
                memcpy(&LightgunButtons::ButtonDesc[FW_Const::BtnIdx_Pedal].reportType,
                       OF_Prefs::backupButtonDesc[FW_Const::BtnIdx_B], sizeof(OF_Prefs::backupButtonDesc[0]));
                break;
            default:
                break;
        }
...
void FW_Common::UpdateStartSelect() {
    ...
    for (int i = 0; i < ButtonCount; ++i) {
        do {
            btnMatchedPtr = (uint8_t *)memchr(&LightgunButtons::ButtonDesc[i].reportCode, 0xFF,
                                              sizeof(OF_Prefs::backupButtonDesc[i]) - 1);
            if (btnMatchedPtr != nullptr)
                *btnMatchedPtr = playerStartBtn;
        } while (btnMatchedPtr != nullptr);

        do {
            btnMatchedPtr = (uint8_t *)memchr(&LightgunButtons::ButtonDesc[i].reportCode, 0xFE,
                                              sizeof(OF_Prefs::backupButtonDesc[i]) - 1);
            if (btnMatchedPtr != nullptr)
