                ...
                switch (OFPresets.profSettingTypes_Strings.at(buf)) {
                    ...
                    default:
                        ...
                        profileNum < PROFILE_COUNT
                            ? prefsFile.readBytes((char *)&profiles[profileNum] +
                                                      (sizeof(uint32_t) * OFPresets.profSettingTypes_Strings.at(buf)),
                                                  readSize)
...
int OF_Prefs::SaveToPtr(File prefsFile, void *dataPtr, const std::unordered_map<std::string_view, int> &mapPtr,
                        const size_t &dataSize) {
    if (prefsFile) {
        for (auto &pair : mapPtr) {
            if ((pair.second >= 0 && dataPtr != backupButtonDesc) ||
                dataPtr == backupButtonDesc && pair.second >= 0 && pair.second < ButtonCount) {
                prefsFile.write((const uint8_t *)pair.first.data(), pair.first.length() + 1);
    ...
    if (prefsFile) {
        ...
        while (prefsFile.available()) {
            ...
            if (bWritten && mapPtr.count(buf))
                ...
                prefsFile.seek(dataSize, fs::SeekCur);
        }
...
void OF_Prefs::LoadPresets() {
    ...
    for (int i = 0; i < ButtonCount; ++i)
        memcpy(OF_Prefs::backupButtonDesc[i], &LightgunButtons::ButtonDesc[i].reportType,
               sizeof(OF_Prefs::backupButtonDesc[i]));
}
