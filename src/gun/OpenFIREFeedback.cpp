...
void OF_FFB::FFBOnScreen() {
    if (OF_Prefs::toggles[OF_Const::solenoid]) {  // (Only activate when the solenoid switch is on!)
        ...
        if (!triggerHeld) {
            ...
            SolenoidActivation(digitalRead(OF_Prefs::pins[OF_Const::solenoidPin])
                                   ? OF_Prefs::settings[OF_Const::solenoidOnLength]
                                   : OF_Prefs::settings[OF_Const::solenoidOffLength] << autofireDoubleLengthWait);
        } else if (solenoidFirstShot) {  // If we aren't in autofire mode, are we waiting for the initial shot timer
                                         // still?
            if (digitalRead(
                    OF_Prefs::pins[OF_Const::solenoidPin])) {  // If so, are we still engaged? We need to let it go
                                                               // normally, but maintain the single shot flag.
        ...
        } else if (!burstFireActive) {  // if we don't have the single shot wait flag on (holding the trigger w/out
                                        // autofire)
            SolenoidActivation(digitalRead(OF_Prefs::pins[OF_Const::solenoidPin])
                                   ? OF_Prefs::settings[OF_Const::solenoidOnLength]
                                   : OF_Prefs::settings[OF_Const::solenoidOffLength]);
        }
        // only activate rumbleFF as a fallback if Solenoid is explicitly disabled
    } else if (OF_Prefs::toggles[OF_Const::rumble] && OF_Prefs::toggles[OF_Const::rumbleFF] && !rumbleHappened &&
               !triggerHeld)
        RumbleActivation();

    if (OF_Prefs::toggles[OF_Const::rumble] && rumbleHappening && triggerHeld)
        RumbleActivation();  // Continue processing the rumble command, to prevent infinite rumble while going from
                             // on-screen to off mid-command.
}

void OF_FFB::FFBOffScreen() {
    if (OF_Prefs::toggles[OF_Const::rumble]) {
        // Is this the first time we're rumbling AND only started pulling the trigger (to prevent starting a rumble w/
        // trigger hold)?
    ...
    if (burstFiring)
        BurstFire();
    else if (digitalRead(OF_Prefs::pins[OF_Const::solenoidPin]) &&
             !burstFireActive) {  // If the solenoid is engaged, since we're not shooting the screen, shut off the
                                  // solenoid a'la an idle cycle
...
void OF_FFB::FFBRelease() {
    if (OF_Prefs::toggles[OF_Const::solenoid]) {
        if (burstFiring)
            ...
            if (currentMillis - previousMillisSol >=
                OF_Prefs::settings[OF_Const::solenoidOnLength]) {  // I guess if we're not firing, may as well use the
                                                                   // fastest shutoff.
    ...
    // If Rumble FF is enabled and Autofire is enabled, the motor needs to be disabled when the trigger is released.
    // Otherwise, allow RumbleActivation to deal with the activation timer
    if (OF_Prefs::toggles[OF_Const::rumbleFF] && OF_Prefs::toggles[OF_Const::autofire]) {
        if (rumbleHappening || rumbleHappened) {
#ifdef ARDUINO_ARCH_ESP32
            analogWrite(OF_Prefs::pins[OF_Const::rumblePin], 0);  // Make sure the rumble is OFF. // 696969 for esp32
#else                                                             // rp2040
        ...
        if (rumbleHappening) {   // Are we currently in a rumble command? (Implicitly needs
                                 // OF_Prefs::toggles[OF_Const::rumble])
            RumbleActivation();  // Continue processing our rumble command.
            // (This is to prevent making the lack of trigger pull actually activate a rumble command instead of
            // skipping it like we should.)
...
void OF_FFB::SolenoidActivation(const int& solenoidFinalInterval) {
    if (solenoidFirstShot) {  // If this is the first time we're shooting, it's probably safe to shoot regardless of
                              // temps.
        previousMillisSol = millis();  // Calibrate the timer for future calcs.
        digitalWrite(OF_Prefs::pins[OF_Const::solenoidPin],
                     HIGH);  // Since we're shooting the first time, just turn it on aaaaand fire.
    } else {
                ...
                if (tempStatus == Temp_Warning) {
                    if (digitalRead(OF_Prefs::pins[OF_Const::solenoidPin])) {  // Is the valve being pulled now?
                        if (currentMillis - previousMillisSol >= solenoidFinalInterval) {
                            previousMillisSol = currentMillis;
                            digitalWrite(OF_Prefs::pins[OF_Const::solenoidPin],
                                         !digitalRead(OF_Prefs::pins[OF_Const::solenoidPin]));  // Flip, flop.
                        }
                    } else {  // The solenoid's probably off right now, so that means we should wait a bit longer to
                              // fire again.
                        if (currentMillis - previousMillisSol >=
                            (OF_Prefs::settings[OF_Const::solenoidOffLength]
                             << 1)) {  // We're keeping it low for a bit longer, to keep temps stable. Try to give it a
                                       // bit of time to cool down before we go again.
                            previousMillisSol = currentMillis;
                            digitalWrite(OF_Prefs::pins[OF_Const::solenoidPin],
                                         !digitalRead(OF_Prefs::pins[OF_Const::solenoidPin]));
                        }
                    }
                } else {
                    if (currentMillis - previousMillisSol >= solenoidFinalInterval) {
                        previousMillisSol = currentMillis;
                        digitalWrite(
                            OF_Prefs::pins[OF_Const::solenoidPin],
                            !digitalRead(OF_Prefs::pins[OF_Const::solenoidPin]));  // run the solenoid into the state
                                                                                   // we've just inverted it to.
                    }
                }
            } else {
#ifdef PRINT_VERBOSE
...
#endif
                digitalWrite(OF_Prefs::pins[OF_Const::solenoidPin],
                             LOW);  // Make sure it's off if we're this dangerously close to the sun.
            }
        } else {  // No temp sensor, so just go ahead.
            currentMillis = millis();
            if (currentMillis - previousMillisSol >=
                solenoidFinalInterval) {            // If we've waited long enough for this interval,
...
void OF_FFB::TemperatureUpdate() {
    ...
    if (currentMillis - previousMillisTemp > TEMP_UPDATE_INTERVAL) {
        ...
        temperatureGraph[tempGraphIndex] = (((analogRead(OF_Prefs::pins[OF_Const::tempPin]) * 3.3) / 4096) - 0.5) *
                                           100;  // Convert reading from mV->3.3->12-bit->Celsius
        if (tempGraphIndex < 3) {
            tempGraphIndex++;
        } else {
            ...
            tempGraphIndex = 0;
            temperatureCurrent =
                (temperatureGraph[0] + temperatureGraph[1] + temperatureGraph[2] + temperatureGraph[3]) >> 2;
...
void OF_FFB::RumbleActivation() {
    if (rumbleHappening) {         // Are we in a rumble command rn?
        ...
        if (OF_Prefs::toggles[OF_Const::rumbleFF]) {
            if (!OF_Prefs::toggles[OF_Const::autofire]) {  // We only want to use the rumble timer if Autofire is not
                                                           // active. Otherwise, keep it going
                if (currentMillis - previousMillisRumble >=
                    (OF_Prefs::settings[OF_Const::rumbleInterval] >>
                     1)) {  // If we've been waiting long enough for this whole rumble command,
#ifdef ARDUINO_ARCH_ESP32
                    analogWrite(OF_Prefs::pins[OF_Const::rumblePin],
                                0);  // Make sure the rumble is OFF. // 696969 for ESP32
#else                                // rp2040
                    digitalWrite(OF_Prefs::pins[OF_Const::rumblePin], LOW);  // Make sure the rumble is OFF.
#endif
                    rumbleHappening = false;  // This rumble command is done now.
            ...
            if (currentMillis - previousMillisRumble >=
                OF_Prefs::settings[OF_Const::rumbleInterval]) {  // If we've been waiting long enough for this whole
                                                                 // rumble command,
#ifdef ARDUINO_ARCH_ESP32
                analogWrite(OF_Prefs::pins[OF_Const::rumblePin],
                            0);  // Make sure the rumble is OFF. // 696969 for ESP32
...
#ifdef USES_SOLENOID
        if (!digitalRead(OF_Prefs::pins[OF_Const::solenoidPin]) &&  // Is the solenoid NOT on right now, and the counter
                                                                    // hasn't matched?
